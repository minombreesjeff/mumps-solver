  ...
  if(*istk(ljob) == -1){
    ...
    if(*istk(ljob)==-2){
      ...
      if(dmumps_par->rhs && dosolve){ /* Just to know if solution step was called */
        CreateVar(nb_RHS+3,"d",&nA,&nRHS,&lrhsout);
        for(i=0;i<nRHS;i++){
          posrhs=i*nA;
          for(j=0;j<nA;j++){
           *stk(lrhsout+posrhs+j)=(dmumps_par->rhs)[posrhs+j];
          }
        }
        LhsVar(3)=nb_RHS+3;  
      }else{
      ...
      if(temp4>0){
     CreateVar(nb_RHS+5,"d",&temp4,&temp4,&lschurout);
      for(i=0;i<temp4;i++){
      posschur=i*temp4;
        for(j=0;j<temp4;j++){
          (stk(lschurout))[posschur+j]=(dmumps_par->schur)[posschur+j];}}
LhsVar(5)=nb_RHS+5;
      }else{
        it=1;
          EXTRACT_CMPLX_FROM_C_TO_SCILAB(5,it,lschurout,lschurouti,(dmumps_par->schur),one,one,one);
