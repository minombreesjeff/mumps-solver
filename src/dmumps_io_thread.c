 ...
 */
void* _dmumps_async_thread_function (void* arg){   
  ...
  int ierr,ret_code;
   ...
   while (!io_flag_stop){
	...
	if(current_io_request->io_type==0){
	  /*write*/	
	  ret_code=dmumps_io_do_write_block(current_io_request->addr,
	  ...
	  if(current_io_request->io_type==1){
	    /*read*/
	    ret_code=dmumps_io_do_read_block(current_io_request->addr,
				     &(current_io_request->size),
				     (current_io_request->pos_in_file),
				     (current_io_request->num_file),
                                     &(current_io_request->file_type),
	    ...
	    printf("Internal error in _dmumps_async_thread_function\n");
	    exit (-3);
	  }
	}
	pthread_mutex_lock(&io_mutex);
 ...
 */
void*  _dmumps_async_thread_function_with_sem (void* arg){
   struct request_io *current_io_request;
   int ierr,_sem_stop;
   struct timeval start_time,end_time;
   int ret_code;
   for (;;){      
     gettimeofday(&start_time,NULL);
       if(with_sem==2){
	 _dmumps_wait_sem(&int_sem_io,&cond_io);
       }
     ...
     /*     sem_getvalue(&sem_stop,&_sem_stop); */
       if(with_sem==2){
	 _dmumps_get_sem(&int_sem_stop,&_sem_stop);
       }
	...
	case IO_READ:
	 ...
	 default:
      ...
      /* Wait that finished_requests queue could register 
       the notification */
       if(with_sem==2){
	 _dmumps_wait_sem(&int_sem_nb_free_finished_requests,&cond_nb_free_finished_requests);
       }
	 ...
	 first_active=0;
      ...
      if(with_sem==2){
	_dmumps_post_sem(&(current_io_request->int_local_cond),&(current_io_request->local_cond));
      }
	  ...
	  _dmumps_post_sem(&int_sem_nb_free_active_requests,&cond_nb_free_active_requests);
...
int dmumps_is_there_finished_request_th(int* flag){
  if(!dmumps_owns_mutex) pthread_mutex_lock(&io_mutex);
  ...
  if(!dmumps_owns_mutex) pthread_mutex_unlock(&io_mutex);
  return 0;
}

int dmumps_clean_request_th(int* request_id){
  int ierr;
  ...
  *request_id=finished_requests_id[first_finished_requests];
  if(smallest_request_id!=finished_requests_id[first_finished_requests]){
    sprintf(error_str,"Internal error in OOC Management layer (dmumps_clean_request_th)\n");
    return -91;
  }
  ...
  if(with_sem) {
      if(with_sem==2){
	_dmumps_post_sem(&int_sem_nb_free_finished_requests,&cond_nb_free_finished_requests);
      }
...
int _dmumps_clean_finished_queue_th(){
  ...
  /* this block of code is designed for avoiding deadlocks between
     the two threads*/
   dmumps_is_there_finished_request_th(&local_flag);
   while(local_flag){
     ierr=dmumps_clean_request_th(&cur_req);
     if(ierr!=0){
       return ierr;
     }
     dmumps_is_there_finished_request_th(&local_flag);
   }
   if((!dmumps_owns_mutex)||(loc_owned_mutex)){
      pthread_mutex_unlock(&io_mutex);
      dmumps_owns_mutex=0;
   }
   return 0;
...
int dmumps_test_request_th(int* request_id,int *flag){
  ...
  if(*request_id < smallest_request_id){
    *flag=1;
	...
	if(i==nb_active){
	  sprintf(error_str,"Internal error in OOC Management layer (dmumps_test_request_th (1))\n");
	  return -91;
	}
	*flag=0;
      }else{
	...
	if(i==nb_finished_requests){
	  sprintf(error_str,"Internal error in OOC Management layer (dmumps_test_request_th (2))\n");
	  return -91;
	}      
	*flag=1;
      }
  ...
  _dmumps_clean_finished_queue_th();
  dmumps_owns_mutex=0;
  pthread_mutex_unlock(&io_mutex);
...
int dmumps_wait_req_sem_th(int *request_id){
  int i,j;
  j=first_active;
  ...
  if(i<nb_active){
    _dmumps_wait_sem(&(io_queue[j].int_local_cond),&(io_queue[j].local_cond));
  }
  return 0;
}

int dmumps_wait_request_th(int *request_id){
  ...
  if(with_sem!=2){
    ...
    if(ierr!=0)return ierr;
    if(!flag){
      dmumps_wait_req_sem_th(request_id);
      ierr=dmumps_test_request_th(request_id,&flag);
      if(ierr!=0)return ierr;
    }
  }
...
int dmumps_wait_all_requests_th(){
  ...
  while((nb_active!=0)||(nb_finished_requests!=0)){
    pthread_mutex_unlock(&io_mutex);
    dmumps_is_there_finished_request_th(&flag);
    if(flag){
      ierr=dmumps_clean_request_th(&flag);
      if(ierr!=0){
	return ierr;
      }
    }else{
...
int dmumps_low_level_init_ooc_c_th(int* async, int* ierr){
  ...
  if(*async!=IO_ASYNC_TH){
    sprintf(error_str,"Internal error: dmumps_low_level_init_ooc_c_th should not to be called with strat_IO=%d\n",*async);
    return -91;
  }
  if(*async){
	...
	return -92;
      ...
      ret_code=pthread_create(&io_thread,NULL,_dmumps_async_thread_function_with_sem,NULL);
    }else{
      ret_code=pthread_create(&io_thread,NULL,_dmumps_async_thread_function,NULL);
    }
    if(ret_code!=0){
      dmumps_io_build_err_str(ret_code,-92,"Unable to create I/O thread",error_str,200);
      return -92;      
    }
...
int dmumps_async_write_th(const int * strat_IO, 
			void * address_block,
			int * block_size,
			int * pos_in_file,
			int * file_number,
  ...
  *ierr=dmumps_check_error_th();
  ...
  if(with_sem){ 
    _dmumps_clean_finished_queue_th();
      if(with_sem==2){
	_dmumps_wait_sem(&int_sem_nb_free_active_requests,&cond_nb_free_active_requests);
      }
  ...
  if(nb_active<=MAX_IO){
    ...
    *request_arg=current_req_num; 
    ...
    sprintf(error_str,"Internal error in OOC Management layer (dmumps_async_write_th)\n");
    return -91;
    /*    exit(-3);*/
  }
  pthread_mutex_unlock(&io_mutex);
  if(with_sem){
    /*    sem_post(&sem_io); */
      if(with_sem==2){
	_dmumps_post_sem(&int_sem_io,&cond_io);
      }
  }
  return 0;
}
...
int dmumps_async_read_th(const int * strat_IO, 
		       void * address_block,
		       int * block_size,
		       int * from_where,
		       int * file_number,
  ...
  *ierr=dmumps_check_error_th();
  ...
  if(nb_active<MAX_IO){
    ...
    *request_arg=current_req_num; 
    ...
    sprintf(error_str,"Internal error in OOC Management layer (dmumps_async_read_th)\n");
    return -91;
  }
  if(with_sem){
    /*    sem_post(&sem_io); */
...
int dmumps_clean_io_data_c_th(int *myid){
  ...
  if(dmumps_io_flag_async){
    ...
    if(with_sem){
	if(with_sem==2){
	  _dmumps_post_sem(&int_sem_stop,&cond_stop);
	  _dmumps_post_sem(&int_sem_io,&cond_io);
	}
    }else{
      pthread_mutex_lock(&io_mutex);
...
int _dmumps_get_sem(void *arg,int *value){
  ...
  case 2:
    ...
    *value=*((int *)arg);
    ...
    break;
  default:
    sprintf(error_str,"Internal error in OOC Management layer (_dmumps__get_sem)\n");
    return -91;
  }
  return 0;
}

int _dmumps_wait_sem(void *arg,pthread_cond_t *cond){
  int *tmp_pointer;
  switch(with_sem){
  case 2:
    pthread_mutex_lock(&io_mutex_cond);
    ...
    break;
  default:
    sprintf(error_str,"Internal error in OOC Management layer (_dmumps_wait_sem)\n");
    return -91;
  }
  return 0;  
}

int _dmumps_post_sem(void *arg,pthread_cond_t *cond){
  int *tmp_pointer;
  switch(with_sem){
  ...
  case 2:
    ...
    break;
  default:
    sprintf(error_str,"Internal error in OOC Management layer (_dmumps_post_sem)\n");
    return -91;
  }
  return 0;  
}
