...
*/
/*    $Id: zmumps_io_basic.h,v 1.50 2006/12/20 09:41:25 aguermou Exp $  */
...
#ifndef _WIN32
#define ZMUMPS_IO_FLAG_O_DIRECT 0
#endif
...
#define ZMUMPS_OOC_DEFAULT_DIR "/tmp"
...
int zmumps_io_do_write_block(void * address_block,int * block_size,int * pos_in_file,int * file_number,int * type,int * ierr);
int zmumps_io_do_read_block(void * address_block,int * block_size,int * from_where,int * file_number,int * type,int * ierr);
...
#if ! defined (_WIN32)
int zmumps_io_write_os_buff__(void *file, void *loc_add, size_t write_size, int where);
int zmumps_io_write_direct_io__(void *file, void *loc_addr, size_t write_size, int where);
int zmumps_io_flush_write__();
#else
int zmumps_io_write_win32__(void *file, void *loc_add, size_t write_size, int where);
#endif
...
int zmumps_io_read_direct_io__(void * file,void * loc_addr,size_t size,int local_offset);
#else
