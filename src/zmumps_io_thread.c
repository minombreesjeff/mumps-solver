...
*/
/*    $Id: zmumps_io_thread.c,v 1.29 2006/09/26 13:15:16 aguermou Exp $  */

#if ! defined (_WIN32) && ! defined (WITHOUT_PTHREAD)  

#include "zmumps_io_basic_extern.h"
#include "zmumps_io_thread_var.h"
  ...
  int ierr,ret_code;
   ...
   while (!io_flag_stop){
	...
	if(current_io_request->io_type==0){
	  /*write*/	
	  ret_code=zmumps_io_do_write_block(current_io_request->addr,
	  ...
	  if(current_io_request->io_type==1){
	    /*read*/
	    ret_code=zmumps_io_do_read_block(current_io_request->addr,
				     &(current_io_request->size),
				     (current_io_request->pos_in_file),
				     (current_io_request->num_file),
                                     &(current_io_request->file_type),
	    ...
	    printf("Internal error in _zmumps_async_thread_function\n");
	    exit (-3);
	  }
	}
	pthread_mutex_lock(&io_mutex);
...
void*  _zmumps_async_thread_function_with_sem (void* arg){
   ...
   for (;;){      
     gettimeofday(&start_time,NULL);
       if(with_sem==2){
	 _zmumps_wait_sem(&int_sem_io,&cond_io);
       }
     ...
     /*     sem_getvalue(&sem_stop,&_sem_stop); */
       if(with_sem==2){
	 _zmumps_get_sem(&int_sem_stop,&_sem_stop);
       }
	...
	case IO_READ:
	 ...
	 default:
      ...
      /* Wait that finished_requests queue could register 
       the notification */
       if(with_sem==2){
	 _zmumps_wait_sem(&int_sem_nb_free_finished_requests,&cond_nb_free_finished_requests);
       }
	...
	_zmumps_post_sem(&(current_io_request->int_local_cond),&(current_io_request->local_cond));
	  ...
	  _zmumps_post_sem(&int_sem_nb_free_active_requests,&cond_nb_free_active_requests);
...
int zmumps_is_there_finished_request_th(int* flag){
  if(!zmumps_owns_mutex) pthread_mutex_lock(&io_mutex);
  ...
  if(!zmumps_owns_mutex) pthread_mutex_unlock(&io_mutex);
  return 0;
}

int zmumps_clean_request_th(int* request_id){
  int ierr;
  ierr=zmumps_check_error_th();
  if(ierr!=0){
    return ierr;
  }
  if(!zmumps_owns_mutex)pthread_mutex_lock(&io_mutex);
  *request_id=finished_requests_id[first_finished_requests];
  if(smallest_request_id!=finished_requests_id[first_finished_requests]){
    sprintf(error_str,"Internal error in OOC Management layer (zmumps_clean_request_th)\n");
    return -91;
  }
  ...
  if(with_sem) {
      if(with_sem==2){
	_zmumps_post_sem(&int_sem_nb_free_finished_requests,&cond_nb_free_finished_requests);
      }
...
int _zmumps_clean_finished_queue_th(){
  ...
  /* this block of code is designed for avoiding deadlocks between
     the two threads*/
   zmumps_is_there_finished_request_th(&local_flag);
   while(local_flag){
     ierr=zmumps_clean_request_th(&cur_req);
     if(ierr!=0){
       return ierr;
     }
     zmumps_is_there_finished_request_th(&local_flag);
   }
   if((!zmumps_owns_mutex)||(loc_owned_mutex)){
      pthread_mutex_unlock(&io_mutex);
      zmumps_owns_mutex=0;
   }
   return 0;
...
int zmumps_test_request_th(int* request_id,int *flag){
  ...
  if(*request_id < smallest_request_id){
    *flag=1;
	...
	if(i==nb_active){
	  sprintf(error_str,"Internal error in OOC Management layer (zmumps_test_request_th (1))\n");
	  return -91;
	}
	*flag=0;
      }else{
	...
	if(i==nb_finished_requests){
	  sprintf(error_str,"Internal error in OOC Management layer (zmumps_test_request_th (2))\n");
	  return -91;
	}      
	*flag=1;
      }
  ...
  _zmumps_clean_finished_queue_th();
  zmumps_owns_mutex=0;
  pthread_mutex_unlock(&io_mutex);
...
int zmumps_wait_all_requests_th(){
  ...
  while((nb_active!=0)||(nb_finished_requests!=0)){
    pthread_mutex_unlock(&io_mutex);
    zmumps_is_there_finished_request_th(&flag);
    if(flag){
      ierr=zmumps_clean_request_th(&flag);
      if(ierr!=0){
	return ierr;
      }
    }else{
...
int zmumps_low_level_init_ooc_c_th(int* async, int* ierr){
  int ret_code;    
  int i;
  ...
  if(*async!=IO_ASYNC_TH){
    sprintf(error_str,"Internal error: zmumps_low_level_init_ooc_c_th should not to be called with strat_IO=%d\n",*async);
    return -91;
  }
  if(*async){
    pthread_mutex_init(&io_mutex,NULL);
    zmumps_io_init_err_lock();
#ifdef WITH_PFUNC
    zmumps_io_init_pointers_lock();
#endif
	...
	return -92;
      ...
      ret_code=pthread_create(&io_thread,NULL,_zmumps_async_thread_function_with_sem,NULL);
    }else{
      ret_code=pthread_create(&io_thread,NULL,_zmumps_async_thread_function,NULL);
    }
    if(ret_code!=0){
      zmumps_io_build_err_str(ret_code,-92,"Unable to create I/O thread",error_str,200);
      return -92;      
    }
...
int zmumps_async_write_th(const int * strat_IO, 
			void * address_block,
			int * block_size,
			int * pos_in_file,
			int * file_number,
  ...
  *ierr=zmumps_check_error_th();
  ...
  if(with_sem){ 
    _zmumps_clean_finished_queue_th();
      if(with_sem==2){
	_zmumps_wait_sem(&int_sem_nb_free_active_requests,&cond_nb_free_active_requests);
      }
  ...
  if(nb_active<=MAX_IO){
    ...
    *request_arg=current_req_num; 
    ...
    sprintf(error_str,"Internal error in OOC Management layer (zmumps_async_write_th)\n");
    return -91;
    /*    exit(-3);*/
  }
  pthread_mutex_unlock(&io_mutex);
  if(with_sem){
    /*    sem_post(&sem_io); */
      if(with_sem==2){
	_zmumps_post_sem(&int_sem_io,&cond_io);
      }
  }
  return 0;
}
...
int zmumps_async_read_th(const int * strat_IO, 
		       void * address_block,
		       int * block_size,
		       int * from_where,
		       int * file_number,
  ...
  *ierr=zmumps_check_error_th();
  ...
  if(nb_active<MAX_IO){
    ...
    *request_arg=current_req_num; 
    ...
    sprintf(error_str,"Internal error in OOC Management layer (zmumps_async_read_th)\n");
    return -91;
  }
  if(with_sem){
    /*    sem_post(&sem_io); */
...
int zmumps_clean_io_data_c_th(int *myid){
  ...
  if(zmumps_io_flag_async){
    ...
    if(with_sem){
	if(with_sem==2){
	  _zmumps_post_sem(&int_sem_stop,&cond_stop);
	  _zmumps_post_sem(&int_sem_io,&cond_io);
	}
    }else{
      pthread_mutex_lock(&io_mutex);
    ...
    zmumps_io_destroy_err_lock();
#ifdef WITH_PFUNC
    zmumps_io_destroy_pointers_lock();
#endif
...
int _zmumps_get_sem(void *arg,int *value){
  ...
  case 2:
    ...
    *value=*((int *)arg);
    ...
    break;
  default:
    sprintf(error_str,"Internal error in OOC Management layer (_zmumps__get_sem)\n");
    return -91;
  }
  return 0;
}

int _zmumps_wait_sem(void *arg,pthread_cond_t *cond){
  int *tmp_pointer;
  switch(with_sem){
  case 2:
    pthread_mutex_lock(&io_mutex_cond);
    ...
    break;
  default:
    sprintf(error_str,"Internal error in OOC Management layer (_zmumps_wait_sem)\n");
    return -91;
  }
  return 0;  
}

int _zmumps_post_sem(void *arg,pthread_cond_t *cond){
  int *tmp_pointer;
  switch(with_sem){
  ...
  case 2:
    ...
    break;
  default:
    sprintf(error_str,"Internal error in OOC Management layer (_zmumps_post_sem)\n");
    return -91;
  }
  return 0;  
}
