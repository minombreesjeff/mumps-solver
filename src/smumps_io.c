...
void MUMPS_CALL smumps_low_level_init_prefix(int * dim, char * str, smumps_ftnlen l1){
  int i;
  smumps_ooc_store_prefixlen=*dim;
  if (*dim>150) smumps_ooc_store_prefixlen=150;
...
void MUMPS_CALL smumps_low_level_init_tmpdir(int * dim, char * str, smumps_ftnlen l1){
  int i;
  smumps_ooc_store_tmpdirlen=*dim;
  if (*dim>150) smumps_ooc_store_tmpdirlen=150;
...
/*   smumps_io_last_file_opened=-1; */
  ...
  smumps_io_k211=*k211;
  if (smumps_ooc_store_prefixlen==-1) {
    printf(error_str,"Error: prefix not initialized\n");
    *ierr=-92;
    smumps_io_prop_err_info(*ierr);
    return;
  }
  if (smumps_ooc_store_tmpdirlen==-1) {
    printf(error_str,"Error: tmpdir not initialized\n");
    *ierr=-92;
    smumps_io_prop_err_info(*ierr);
    return;
  }
  *ierr=smumps_init_file_name(smumps_ooc_store_tmpdir, smumps_ooc_store_prefix,
		             &smumps_ooc_store_tmpdirlen, &smumps_ooc_store_prefixlen, _myid);
  if(*ierr<0){
    smumps_io_prop_err_info(*ierr);
    return;
  }
   ...
   */
  smumps_ooc_store_prefixlen=-1;
  smumps_ooc_store_tmpdirlen=-1;
...
void MUMPS_CALL smumps_clean_io_data_c(int * myid,int *step,int *ierr){
  ...
  smumps_free_file_pointers(step);
  smumps_io_is_init_called=0;
...
void MUMPS_CALL smumps_ooc_get_nb_files(int* nb_files){
  int tmp=0;
  int ret;
  ret=smumps_io_get_nb_files(nb_files,&tmp);
  return;
}
...
void MUMPS_CALL smumps_ooc_get_file_name(int* indice,int* length, char* name, smumps_ftnlen l1){
  int tmp=0;
  int ret;
  ret=smumps_io_get_file_name(indice,name,length,&tmp);
  return;
}

void MUMPS_CALL smumps_ooc_set_file_name(int* indice,int* length, int *ierr, char* name, smumps_ftnlen l1){
  int tmp=0;
  *ierr=smumps_io_set_file_name(indice,name,length,&tmp);
  if(*ierr<0){
    smumps_io_prop_err_info(*ierr);
  }
  return;
...
void MUMPS_CALL smumps_ooc_alloc_pointers(int* dim,int* ierr){
  ...
  *ierr=ret_code;
  ret_code=smumps_io_set_last_file(dim,&tmp);
  return;
}
...
#endif
  smumps_io_k211=*k211;
  *ierr=smumps_io_init_vars(myid_arg, nb_file_arg,size_element,async);
  return;
}
...
/*  smumps_io_flush_write__ is not defined in
