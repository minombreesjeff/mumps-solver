...
*/
/*    $Id: dmumps_io.c,v 1.55 2006/09/26 13:15:16 aguermou Exp $    */
...
void MUMPS_CALL dmumps_low_level_write_ooc_c(const int * strat_IO, 
   ...
   if(dmumps_io_flag_async){
...
#if ! defined (_WIN32) && ! defined (WITHOUT_PTHREAD)
     case IO_ASYNC_TH:
       ret_code=dmumps_async_write_th(strat_IO, address_block, block_size,pos_in_file,file_number,inode,request_arg,type,ierr);       
       if(ret_code<0){
         *ierr=ret_code;
     ...
     ret_code=dmumps_io_do_write_block(address_block,block_size,pos_in_file,file_number,type,ierr);   
     if(ret_code<0){
       *ierr=ret_code;
       dmumps_io_prop_err_info(ret_code);
     }
...
void MUMPS_CALL dmumps_low_level_read_ooc_c(const int * strat_IO, 
  ...
  if(dmumps_io_flag_async){
...
#if ! defined (_WIN32) && ! defined (WITHOUT_PTHREAD)
      case IO_ASYNC_TH:
        ret_code=dmumps_async_read_th(strat_IO,address_block,block_size,from_where,file_number,inode,request_arg,type,ierr);
        if(ret_code<0){
          *ierr=ret_code;
          dmumps_io_prop_err_info(ret_code);
        }
    ...
    ret_code=dmumps_io_do_read_block(address_block,block_size,from_where,file_number,type,ierr);
    if(ret_code<0){
      *ierr=ret_code;
      dmumps_io_prop_err_info(ret_code);
    }
